...
bool VNetRouteOrch::addTunnelRoutePost(const TunnelRouteContext& tr_ctx)
{
    ...
    if (it_route != syncd_tunnel_routes_[tr_ctx.vnet].end() &&
        ((tr_ctx.monitoring == "" && it_route->second.nhg_key != tr_ctx.nhg) ||
        ((tr_ctx.monitoring == VNET_MONITORING_TYPE_CUSTOM || tr_ctx.monitoring == VNET_MONITORING_TYPE_CUSTOM_BFD) && (it_route->second.primary != tr_ctx.primary || it_route->second.secondary != tr_ctx.secondary))))
    {
        route_updated = true;
